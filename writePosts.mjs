 ...
 *  console.log(`${performance.now() - start}ms - ${id}`)
 */
...
;(async function writePosts() {
  ...
  if (fs.existsSync(POSTS_DIR)) {
    ...
    await Promise.all(files.map(file => fs.promises.unlink(path.join(POSTS_DIR, file))))
  } else {
    await fs.promises.mkdir(POSTS_DIR)
  }

  await Promise.all(Array.from(Array(numPosts).keys()).map(idx => {
    return fs.promises.writeFile(`${POSTS_DIR}/post-${idx}.mdx`, toMdContents(idx))
  })
  )
